...
> 当前 Docker 方案为**单端口架构**：容器内仅运行 Express（`60000`），同时提供 API 与前端静态页面，对外统一暴露 `80`。
...
| 端口    | 用途                    | 在线部署映射    | 本地构建映射      |
| ------- | ----------------------- | --------------- | ----------------- |
| `80`    | 统一入口（前端 + API）  | `80:60000`      | `80:60000`        |
| `60000` | 容器内 Express 服务端口 | 仅容器内部使用  | 仅容器内部使用    |
...
3. **启动开发环境**
   ...
   - **方式一：启动 Web 服务（开发调试用）**
     ...
     > `yarn dev` 默认启动 `60000` 端口的 Express 服务。若 `scripts/web` 目录存在（例如已放入前端构建产物），可直接通过 `http://localhost:60000` 访问前端页面并同源调用 API。
   ...
   **两种模式对比：**
   ...
   | 命令           | 启动内容                             | 前端页面              | 适用场景                         |
   | -------------- | ------------------------------------ | --------------------- | -------------------------------- |
   | `yarn dev`     | Express Web 服务（端口 60000）       | ✅（需 `scripts/web`） | Web 同源联调、后端调试           |
