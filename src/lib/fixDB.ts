...
export default async (knex: Knex): Promise<void> => {
  ...
  const createPromptGovernanceTables = async () => {
    if (!(await knex.schema.hasTable("o_promptVersion"))) {
      await knex.schema.createTable("o_promptVersion", (table) => {
    ...
    if (!(await knex.schema.hasTable("o_promptUsage"))) {
      await knex.schema.createTable("o_promptUsage", (table) => {
        table.integer("id").notNullable();
        table.string("scope");
        table.string("key");
  ...
  const createGenerationAuditTables = async () => {
    if (!(await knex.schema.hasTable("o_generationArtifact"))) {
      await knex.schema.createTable("o_generationArtifact", (table) => {
        table.integer("id").notNullable();
    ...
    if (!(await knex.schema.hasTable("o_generationSegment"))) {
      await knex.schema.createTable("o_generationSegment", (table) => {
        table.integer("id").notNullable();
    ...
    if (!(await knex.schema.hasTable("o_generationRevision"))) {
      await knex.schema.createTable("o_generationRevision", (table) => {
        table.integer("id").notNullable();
        table.integer("artifactId");
  ...
  const createCreativeCanvasTables = async () => {
    if (!(await knex.schema.hasTable("o_creativeCanvasState"))) {
      await knex.schema.createTable("o_creativeCanvasState", (table) => {
        table.integer("id").notNullable();
    ...
    if (!(await knex.schema.hasTable("o_agentChatHistory"))) {
      await knex.schema.createTable("o_agentChatHistory", (table) => {
        table.integer("id").notNullable();
        table.integer("projectId").notNullable();
        table.integer("scriptId");
  ...
  const createTaskProgressTables = async () => {
    if (!(await knex.schema.hasTable("o_taskProgress"))) {
      await knex.schema.createTable("o_taskProgress", (table) => {
        table.integer("id").notNullable();
  ...
  const recoverInterruptedTasks = async () => {
    ...
    if (await knex.schema.hasTable("o_taskProgress")) {
      const recoveredRows = await Promise.all(
        runningTasks.map(async (task: any) => {
          const exists = await knex("o_taskProgress").where("taskId", task.id).where("phase", "recovered").first();
          if (exists) return null;
          let related: any = {};
          try {
            related = JSON.parse(task.relatedObjects || "{}");
          } catch {
            related = {};
          }
          return {
            ...
            scriptId: related.scriptId ?? (Array.isArray(related.scriptIds) ? related.scriptIds[0] : null),
            phase: "recovered",
            status: "error",
            message: "软件重启导致进行中任务中断，请重新提交",
            current: null,
            total: null,
            meta: JSON.stringify({ related }),
            createTime: now,
            updateTime: now,
          };
        }),
      );
      const rows = recoveredRows.filter(Boolean);
      if (rows.length) await knex("o_taskProgress").insert(rows);
    }
    await knex("o_tasks").where("state", "进行中").update({
      state: "生成失败",
      reason: "软件重启导致进行中任务中断，请重新提交",
    });
  };
  //矫正因软件异常退出导致的状态不一致问题
  ...
  if (await knex.schema.hasTable("o_genQueue")) {
    await knex("o_genQueue").where("state", "执行中").update({
      state: "排队中",
      updateTime: Date.now(),
    });
  }
  ...
  if (await knex.schema.hasTable("o_videoCompose")) {
    await knex("o_videoCompose").where("state", "合成中").update({
      state: "合成失败",
      errorReason: "软件退出导致失败",
    });
  }
  if (await knex.schema.hasTable("o_episodeMerge")) {
    await knex("o_episodeMerge").where("state", "拼接中").update({
      state: "拼接失败",
      errorReason: "软件退出导致失败",
    });
  }
  ...
  await addColumn("o_tasks", "promptSource", "text");
  // 抽卡候选组字段
