...
describe("productionAgent/index.ts — model-facing text follows prompt_language, person-facing name labels follow content_language", () => {
  ...
  // đọc thẳng bản gốc .md (zh). production_agent_decision.md không có sidecar trong beforeEach,
  ...
  describe("F2 — skill system prompt (model-facing) follows prompt_language", () => {
    beforeEach(() => {
      fs.writeFileSync(path.join(tmpRoot, "skills", "production_agent_decision.en.md"), "EN_SYSTEM_SKILL", "utf-8");
      fs.writeFileSync(path.join(tmpRoot, "skills", "production_agent_decision.vi.md"), "VI_SYSTEM_SKILL", "utf-8");
    });

    it("prompt_language=en -> system prompt đến từ production_agent_decision.en.md, không phải bản gốc zh", async () => {
      await setSetting("content_language", "vi");
      await setSetting("prompt_language", "en");
      const { runDecisionAI } = await import("./index");
      const resTool = makeResTool();
    ...
    it("prompt_language=vi -> system prompt đến từ production_agent_decision.vi.md, kể cả khi content_language=en", async () => {
      await setSetting("content_language", "en");
      await setSetting("prompt_language", "vi");
      const { runDecisionAI } = await import("./index");
      const resTool = makeResTool();
    ...
    it("prompt_language không có sidecar (zh) -> lùi về bản gốc production_agent_decision.md", async () => {
      await setSetting("content_language", "en");
