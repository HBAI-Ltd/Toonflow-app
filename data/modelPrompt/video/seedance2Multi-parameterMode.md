...
分镜以 `<storyboardItem>` 标签传入，**每个 `<storyboardItem>` 代表一「组」**，包含核心属性与可选首帧参考属性：

```xml
<storyboardItem
  videoDesc='[承接上镜：……（若有）] | 该组分镜行原文：序号1 | {画面描述} | {时长} | {景别} | {运镜} | {台词} | {音效} | 序号2 | …'
  duration='该组总时长'
  keyframePrompt='可选：已选中分镜图对应的单张静态关键帧提示词'
  selectedStoryboardImage='true/false'
></storyboardItem>
```
...
| `keyframePrompt` | 可选首帧/空间底盘参考；只作为当前组起始画面约束，不替代 `videoDesc` | 上游分镜图提示词 |
| `selectedStoryboardImage` | 是否已有 QA 合格并被选中的分镜图；`true` 表示生成请求可能携带该图作为视觉参考 | 上游分镜图状态 |

> 本模式允许“资产参考图 + 已选中分镜图”并存：资产图定义角色/场景/道具，已选中分镜图只用于首帧构图和空间关系锚定。不要把这视为模型配置冲突。

---

## 任务目标

读取所有 `<storyboardItem>` 的 `videoDesc`，将其拆为各 `序号N` 子镜头，结合资产信息和可选首帧参考，按 Seedance 2.0 文本多参语法，把全部镜头整合为**一个完整的视频提示词**（非逐条独立）。资产图负责主体定义；已选中分镜图负责当前组首帧/空间底盘，不替代剧情动作。
...
- **镜头连续性合同**：若 `<storyboardItem>` 含「镜头连续性合同」，用它约束固定空间、人物身份、道具关系和 QA 禁止项；若合同与 `videoDesc` 的明确剧情动作冲突，以 `videoDesc` 为准，未明确改变的关系继续锁定。
...
> 若 `selectedStoryboardImage='true'`，可在主体定义后追加一句首帧锚定说明，使用 `keyframePrompt` 概括空间关系；不要输出模型配置判断或输入冲突说明。
...
5. **分镜图可选**：`selectedStoryboardImage='true'` 时，允许把已选中分镜图当作首帧/空间底盘参考；`selectedStoryboardImage='false'` 时不得虚构分镜图引用。`@图片N` 的主体定义仍只绑定资产图，不把分镜图定义为角色/场景/道具。
