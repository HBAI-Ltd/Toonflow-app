...
8. **复审必须收敛**：若输入中提供上一轮审核报告，先逐项核销旧问题。只有本轮修改新引入的回归，或此前漏检且确实适用的 R1~R4 硬红线，才允许新增为严重问题；此前未报告的审美/优化项不得升级为新的验收门槛。
...
- 字段完整性（场头、片段引用资产、每镜均保留 画面描述/时长/景别/运镜/台词/音效 列；没有必要声音时音效单元格允许留空）
...
| 台词完整性 | 严重 | 剧本所有台词（含 OS/VO/系统播报/面板文字）须按实际口播区间写入连续镜的 台词 字段；连续拼接后与原文 100% 一致、标明来源人，无改写/省略/合并/精简；禁止只在第一镜写完整长台词而后续承接镜留空 | R2 |
| 剧本覆盖度与顺序 | 严重 | 剧本场景与关键事件均有对应镜头、无遗漏，无新增剧本外情节，镜头/场次顺序与剧本叙事顺序一致 | R2 |
| 不可拍摄内容已转译 | 严重 | 心理/旁白/抽象交代已转译为可见物象或 OS/VO，未原样塞进 画面描述 | — |
| 禁光影色调 | 严重 | 任何字段（画面描述/运镜/音效/台词来源描述）不得使用表示照明、明暗或色彩的词组（如“光线/打光/逆光/侧光/色温/明暗/色调/暖色/冷色/阴影”）；“身影/背影/剪影/影像”等角色或画面名词不属于此红线 | — |
...
2. 逐条比对各镜 台词 字段，确认长台词按实际口播区间分配到连续镜；按镜头顺序拼接后原文一字不差、标明来源人
...
1. 扫描每镜 画面描述/运镜/音效 及台词来源描述，匹配表示照明、明暗或色彩的词组：光线/打光/逆光/侧光/顶光/色温/明暗/色调/暖色/冷色/冷暖/暖光/冷光/阴影 等；不要把“身影/背影/剪影/影像”按单字“影”误报
