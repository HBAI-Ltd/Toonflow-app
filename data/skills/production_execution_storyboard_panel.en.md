...
|------|------|------|
| `videoDesc` | `string` | Shot description, Scene, Associated asset names, Duration, Shot size, Camera movement, Character action, Emotion, Lighting and atmosphere, Dialogue, Sound effects, Associated asset IDs (**in storyboard-assisted multi-reference mode** this is a fixed text (固定文本)) |
...
Taking each "group" of the storyboard table as a unit, concatenate and write `videoDesc` in the following **fixed order** (固定顺序):
...
```
add_flowData_storyboard({ videoDesc: "the group's video description", prompt: null, track: "the group 序号 accumulated in order", duration: the group's duration, associateAssetsIds: [the scene's cited asset ID list], shouldGenerateImage: "false" })
```

**Step 4 · Finish**
Return one 已完成 (completion) confirmation sentence only: `Storyboard panel writing complete (plain-text multi-reference mode)`.
...
**Step 8 · Finish**
Return one 已完成 (completion) confirmation sentence only: `Storyboard panel writing complete (first/last frame mode)`.
