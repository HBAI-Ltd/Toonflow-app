...
|------|------|------|
| `videoDesc` | `string` | Shot description, Scene, Associated asset names, Duration, Shot size, Camera movement, Character action, Emotion, Lighting and atmosphere, Dialogue, Sound effects, Associated asset IDs (**in storyboard-assisted multi-reference mode** this is a fixed (固定) text) |
...
Taking each "group" of the storyboard table as a unit, concatenate and write `videoDesc` in the following **fixed order** (固定):
