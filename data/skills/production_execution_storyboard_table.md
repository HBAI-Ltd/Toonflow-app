...
- **逐场注意事项**：把该场列出的 `情感砸点 / 一致性锚点 / 空间距离 / 空间连续性 / 易错提示` 逐条落实到具体镜头设计中（情感砸点 → 给到位的景别 / 运镜；一致性锚点 → 跨镜画面内容连续；空间距离 → 站位与景别；空间连续性 → 固定锚点、人物调度、道具位置和视轴线在同场内保持稳定）。`环境音` 仅作氛围理解参考，**本格式无音效字段、不单独成列**。
...
6.  **空间底盘校验**：若导演规划或场景资产卡提供空间连续性约束，先建立该场空间底盘：固定锚点、人物默认站位/座位/朝向、关键道具/陈设位置、视轴线、不变量、允许变化、禁止漂移。后续镜头只改变景别、裁切、机位或人物可见比例，不得无动作依据重排空间。
...
5a. **空间连续性优先**：同一场景内固定空间锚点、关键道具/陈设位置、人物调度和视轴线保持可追踪；镜头变化只改变摄影机观察方式，不把同一场景重排成另一个空间。
