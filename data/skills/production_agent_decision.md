...
阶段4完成后、派发阶段5之前，先判断用户意图是否为整板重做：

| 用户意图 | 决策层操作 |
|----------|-----------|
| 明确要求重做整个分镜面板、覆盖原分镜面板、或清空后重建 | 在派发指令中明确携带 **"整板重做"** 和当前写入模式（纯文本多参模式 / 首位帧模式） |
| 无法明确区分是整板重做还是继续补充（如只说"重新弄一下分镜"） | **先询问用户**是整板重做还是保留后补充，确认前不派发删除任务 |
| 明确要求新增、补充、继续制作分镜 | 保持现有派发规则，不携带"整板重做" |
| 要求重新生成分镜图片 | 派发阶段6，不派发阶段5删除流程 |
| 要求自主识别或删除重复分镜 | **禁止派发**，告知用户明确指定整板重做或取消 |

若为整板重做，写入模式仍需按以下规则确定：
