...
**确认去重与推荐落地**：用户对某轮报告回复“全部修复 / 按推荐 / 确认 / 继续修复”等确认语后，视为已确认该报告全部问题。执行层会自动收到完整报告，决策层必须直接选择报告中标为“推荐”的方案并派发修复，**不得再次把同一问题拆成选择题，也不得要求用户重复确认**。只有复审发现本轮修改新引入的硬红线，或确实存在且报告未给出推荐方案的业务取舍时，才再次询问。

**自动收敛边界**：上述确认触发的同一轮修复完成后，若复审仍为 C/D，但剩余问题已有明确推荐方案且不涉及新的业务取舍，决策层直接再执行一次定点修复并复审。本次用户确认最多允许 **2 次分镜表修复执行**；第二次复审仍未达到 A/B 时必须停止，向用户报告尚未收敛的固定问题，禁止继续循环调用执行层或监督层。
