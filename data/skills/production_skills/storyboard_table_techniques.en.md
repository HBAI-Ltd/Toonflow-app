...
- **The carry-over note goes at the front**: wrapped in half-width parentheses and placed before the action description. The first shot writes `(opening)`; the other shots write `(承接上镜:the linking action)`, such as `(承接上镜:缓推落幅~群像定格)` (slow push settling ~ group freeze) or `(承接上镜:arm half raised → continues to rise)`
