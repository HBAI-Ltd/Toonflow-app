...
- **角色唯一性**：同一 `@图N` 角色在同一张关键帧内只能对应一个真实人物实体。除镜面/水面/玻璃等明确反射场景外，禁止同一角色同时以前景背影、侧面、正面等多个实体重复出现
- **可见人数校验**：正文中所有 role 资产必须按关键帧可见人数逐一绑定；背影、侧脸、虚焦、边缘半截人物都计入人数。若一个角色只是前景背影，也已经占用该角色名额，不得再生成同一角色的正面形象
- **多人构图校验**：多人画面必须明确站位和间距，避免人物重叠、融合、贴脸堆叠、半截裁切或无法辨认。若原分镜包含多段动作，先按首帧识别原则选择一个稳定瞬间，再做人数和站位约束
- **场景空间连续性绑定**：若关联的 scene 资产卡含 `spatialContinuity`、`spatialLayout`、`fixedElements`、`constraints.must/avoid` 等字段，正文必须把它们转译为本镜头可执行的空间锁定描述。锁定对象包括固定空间锚点、人物与固定物关系、关键道具/陈设位置、视轴线、跨镜头不变量；允许变化只限景别、裁切、焦段、机位高度、人物可见比例。禁止把同一场景重排成另一个地点、无依据移动固定物、无动作依据调换人物站位/座位。
