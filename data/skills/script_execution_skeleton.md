...
1. 必须先调用 `get_planData(key="storySkeleton")` 读取当前故事骨架，再调用 `get_novel_events(ids)` 获取事件表。
   - 当前值为空时进入**首次构建模式**，按完整规范创建。
   - 当前值非空且任务要求修改/修复时进入**修复模式**：以当前骨架为底稿，只修改用户已确认项和「上一轮完整审核报告」中的固定问题清单；未点名内容保持不变，除非不改会造成直接矛盾。
   - 修复模式必须逐项核销上一轮报告，不得自行新增改编方向、扩大修改范围或重写整份骨架。
...
- 若【项目配置】为 **1 集且总时长 ≤60 秒**，按微短片处理：不强制大三角、付费卡点、前10集投放、长线人物弧、≈3个股价级反转或“预埋集早于揭晓集”；改用单一因果链、镜头级微反转和完整收束。
...
- 只允许写入 `storySkeleton`，不越权执行其他阶段。
- 不得声称已修改、同步或更新 `adaptationStrategy`；若策略也需变更，由主 Agent 另行调用改编策略执行器。
