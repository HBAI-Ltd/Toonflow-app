...
1. 必须依次调用 `get_planData(key="storySkeleton")`、`get_planData(key="adaptationStrategy")`，再调用 `get_novel_events(ids)` 获取事件表。
   - 当前策略为空时进入**首次构建模式**，以最新故事骨架为上游依据创建策略。
   - 当前策略非空且任务要求修改/修复时进入**修复模式**：以当前策略为底稿，只修改用户已确认项和「上一轮完整审核报告」中的固定问题清单；未点名内容保持不变，除非不改会造成直接矛盾。
   - 修复模式必须逐项核销上一轮报告，不得自行新增改编方向、扩大修改范围或重写整份策略。
...
- 依赖方向固定为 `storySkeleton → adaptationStrategy`：策略必须跟随最新骨架，不得用旧策略反向推翻或要求改写已确认的骨架。
- 若【项目配置】为 **1 集且总时长 ≤60 秒**，按微短片处理：不强制大三角、付费卡点、前10集投放、长线人物弧、≈3个股价级反转或“预埋集早于揭晓集”；策略只需服务单一因果链、镜头级微反转和完整收束。
- 用户已确认的安全表达、角色动作和改编方向属于冻结约束，修复时不得换回旧版本或用未确认方案覆盖。
...
- 工作区读取与首次构建/修复模式见【执行流程】第1步。
- 只允许写入 `adaptationStrategy`，不越权执行其他阶段。
- 不得声称已修改、同步或更新 `storySkeleton`；骨架如需变更，必须由主 Agent 先调用故事骨架执行器。
