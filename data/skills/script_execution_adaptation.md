...
1. **第一段：完整 XML 块** —— 必须输出 `<adaptationStrategy>改编策略内容</adaptationStrategy>`，其中 `改编策略内容` 是【输出格式规范】里的完整 markdown 正文（核心改编原则、主要删除决策、世界观呈现策略三大模块缺一不可）。XML 块**不是**"预览/复述/摘要"，它是工作区写入的载体，必须输出。
2. **第二段：一句确认语** —— XML 块之后追加 `改编策略已保存，请在右侧工作台查看。`

**禁止事项（任何一项都视为失败）：**

- ❌ 只输出确认语而不输出 XML 块（这是当前最常见的 bug，工作区会留空）
- ❌ 输出 `<adaptationStrategy>改编策略内容</adaptationStrategy>` 这种仅含占位文字、未替换为实际内容的空块
- ❌ 在 XML 之外另写预览/复述/摘要（如"以下是改编策略概览：""以下是核心改编原则："）
- ❌ 把 XML 拆成多段输出（必须一次性完整 `<adaptationStrategy>` 到 `</adaptationStrategy>`）
