...
|------|------|------|
| `videoDesc` | `string` | Mô tả hình ảnh, Bối cảnh, Tên tài nguyên liên quan, Thời lượng, Cỡ cảnh, Chuyển động máy quay, Hành động nhân vật, Cảm xúc, Ánh sáng và không khí, Thoại, Hiệu ứng âm thanh, ID tài nguyên liên quan (**chế độ đa tham chiếu có hỗ trợ bảng phân cảnh** thì đây là văn bản cố định) |
...
Lấy mỗi «nhóm» của bảng phân cảnh làm đơn vị, ghép và ghi vào `videoDesc` theo **thứ tự cố định** sau:
...
```
add_flowData_storyboard({ videoDesc: "mô tả video của nhóm đó", prompt: null, track: "số thứ tự nhóm cộng dồn", duration: thời lượng nhóm đó, associateAssetsIds: [danh sách ID tài nguyên trích dẫn của cảnh], shouldGenerateImage: "false" })
```

**Bước 4 · Kết thúc**
Chỉ trả về một câu xác nhận: `Đã ghi xong bảng phân cảnh chi tiết (chế độ đa tham chiếu văn bản thuần)`.
...
**Bước 8 · Kết thúc**
Chỉ trả về một câu xác nhận: `Đã ghi xong bảng phân cảnh chi tiết (chế độ khung đầu-khung cuối)`.
...
- **videoDesc bắt buộc điền**: `videoDesc` của mỗi phân cảnh bắt buộc phải được sinh từ dữ liệu phân cảnh của dòng tương ứng trong `storyboardTable`, chứa đầy đủ thông tin gồm Mô tả hình ảnh, Bối cảnh, Tên tài nguyên liên quan, Thời lượng, Cỡ cảnh, Chuyển động máy quay, Hành động nhân vật, Hướng nhìn, Quan hệ không gian, Cảm xúc, Thoại, Hiệu ứng âm thanh, ID tài nguyên liên quan (**chế độ đa tham chiếu có hỗ trợ bảng phân cảnh là ngoại lệ** —— `videoDesc` là văn bản cố định `参考故事板内容进行视频生成`, thông tin hình ảnh do ảnh bảng phân cảnh gánh)
