...
| `prompt` | `string \| null` | 分镜图片提示词；纯文本多参模式无分镜图时传 `null`；首位帧模式必须传**单张静态关键帧**提示词，禁止复制 `videoDesc` |
| `track` | `string` | 分组 |
...
逐行读取 `storyboardTable` 对应行的「画面描述」「场景」「景别」「角色动作」「朝向」「空间关系」「情绪」字段，严格按已加载技法中的「分镜表内容忠实性原则」和「解析映射规则」将各字段映射为提示词各段落。**prompt 是分镜图提示词，不是视频描述**：必须只描述一个可用于视频首帧的静态关键帧；禁止复制 `videoDesc`；禁止包含「承接上镜」段、编号镜头列表、多段动作过程、运镜、台词或音效；若分镜表行包含连续动作，只取首帧识别原则下的一个凝固瞬间。**提示词正文不得包含光影/色温/明暗/色调描述**。**生成每条提示词后须立即逐字段比对分镜表原始内容**，确认：
