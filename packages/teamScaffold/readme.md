...
不传 `team` 仍是临时子 Agent，无须安装团队。团队使用主 Agent 当前选定的模型和本轮工具，成员上下文独立。每名成员每次最多 12 轮或 10 分钟，一个团队任务最多 24 次成员调用、8 层委派。成员与工具在任务开始时加载，编辑用于后续任务；知识和 Skill 正文使用时读取。当前运行不因禁用插件而自动终止，应通过调用方取消任务。
